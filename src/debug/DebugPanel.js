...
export function installDebugPanel(dev) {
  ...
  const actions = {
    ...
    // Seasons (seasonTools.js — dev.ss.*).
    ss_winter: () => console.log(dev.ss.go('winter')),
    ss_spring: () => console.log(dev.ss.go('spring')),
    ss_autumn: () => console.log(dev.ss.go('autumn')),
    ss_snow: () => console.log(dev.ss.snow(0.9)),
    ss_flood: () => console.log(dev.ss.flood()),
    ss_wood: () => console.log(dev.ss.wood(20)),
    eco_railway: () => console.log(dev.eco.railway()),
    fr_carrier: (sim) => console.log(JSON.stringify(sim.freight.signUp())),
    bt_settype: (sim) => {
      const me = sim.world.toTile(sim.state.player.x, sim.state.player.y);
